...
namespace Facility.ConformanceApi.Testing
{
	...
	public sealed class ConformanceApiService : IConformanceApi
	{
		...
		private ServiceResult<T> Execute<T>(ServiceDto request)
		{
			...
			var actualRequest = ServiceJsonUtility.ToJsonNode(request)?.AsObject();
			var testsWithMatchingRequest = testsWithMethodName.Where(x => ServiceJsonUtility.DeepEquals(x.Request, actualRequest)).ToList();
			...
			{
				var error = ServiceJsonUtility.FromJsonNode<ServiceErrorDto>(testInfo.Error)!;
				var errorRoundTrip = ServiceJsonUtility.ToJsonNode(error);
				if (!ServiceJsonUtility.DeepEquals(testInfo.Error, errorRoundTrip))
			...
			{
				var response = ServiceJsonUtility.FromJsonNode<T>(testInfo.Response)!;
				var responseRoundTrip = ServiceJsonUtility.ToJsonNode(response);
				if (!ServiceJsonUtility.DeepEquals(testInfo.Response, responseRoundTrip))
		...
		private static string Uncapitalize(string value) => string.Concat(value.Substring(0, 1).ToLowerInvariant(), value.AsSpan(1));
