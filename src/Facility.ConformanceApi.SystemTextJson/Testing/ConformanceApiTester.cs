		...
		public ConformanceApiTester(IReadOnlyList<ConformanceTestInfo> tests, IConformanceApi api, HttpClient? httpClient)
		{
			...
			foreach (var testsPerMethod in m_tests.GroupBy(x => x.Method).Select(x => x.ToList()))
			{
				for (var i = 0; i < testsPerMethod.Count; i++)
				{
					...
					{
						if (ServiceJsonUtility.DeepEquals(testsPerMethod[i].Request, testsPerMethod[j].Request))
			...
			try
			{
				if (m_httpClient != null && test.HttpRequest != null)
				{
					...
					if (!httpResponse.IsSuccessStatusCode)
						return Failure($"Got {(int) httpResponse.StatusCode} {httpResponse.ReasonPhrase}: {await httpResponse.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false)}");

					return new ConformanceTestResult(testName, ConformanceTestStatus.Pass);
				}

				static string Capitalize(string value) => string.Concat(value.Substring(0, 1).ToUpperInvariant(), value.AsSpan(1));
				...
				var requestDto = ServiceJsonUtility.FromJsonNode(requestJObject, methodInfo.GetParameters()[0].ParameterType);
				var requestRoundTripJObject = ServiceJsonUtility.ToJsonNode(requestDto);
				if (!ServiceJsonUtility.DeepEquals(requestJObject, requestRoundTripJObject))
				...
				{
					var actualResponseJObject = ServiceJsonUtility.ToJsonNode(actualResponseDto)?.AsObject();
					...
					if (!ServiceJsonUtility.DeepEquals(expectedResponseJObject, actualResponseJObject))
					...
					var expectedResponseDto = (ServiceDto) ServiceJsonUtility.FromJsonNode(expectedResponseJObject, responseType)!;
				...
				else
				{
					...
					var actualErrorJObject = ServiceJsonUtility.ToJsonNode(actualErrorDto)?.AsObject();
					...
					if (!ServiceJsonUtility.DeepEquals(expectedErrorJObject, actualErrorJObject))
					...
					var expectedErrorDto = ServiceJsonUtility.FromJsonNode<ServiceErrorDto>(expectedErrorJObject)!;
