# HttpClientService.TrySendRequestAsync&lt;TRequest,TResponse&gt; method

Sends an HTTP request and processes the response.

```csharp
protected Task<ServiceResult<TResponse>> TrySendRequestAsync<TRequest, TResponse>(
    HttpMethodMapping<TRequest, TResponse> mapping, TRequest request, 
