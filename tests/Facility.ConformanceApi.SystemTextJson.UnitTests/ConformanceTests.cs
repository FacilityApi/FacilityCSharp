...
namespace Facility.ConformanceApi.UnitTests
{
	public class ConformanceTests
	{
		...
		{
			var api = new HttpClientConformanceApi(new HttpClientServiceSettings { HttpClient = s_httpClient, ContentSerializer = new JsonHttpContentSerializer(new() { Serializer = FacilitySerializer.SystemTextJson })});
		...
		{
			using var testsJsonReader = new StreamReader(typeof(ConformanceTests).Assembly.GetManifestResourceStream("Facility.ConformanceApi.SystemTextJson.UnitTests.ConformanceTests.json")!);
		...
		private static HttpClient CreateHttpClient()
		{
			var handler = new ConformanceApiHttpHandler(
					...
					settings: new ServiceHttpHandlerSettings { ContentSerializer = new JsonHttpContentSerializer(new() { Serializer = FacilitySerializer.SystemTextJson })})
