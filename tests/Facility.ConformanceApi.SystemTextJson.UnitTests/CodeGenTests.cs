...
namespace Facility.ConformanceApi.UnitTests
{
	public sealed class CodeGenTests
	{
		...
		{
			string fsdText;
			using (var fsdTextReader = new StreamReader(GetType().Assembly.GetManifestResourceStream("Facility.ConformanceApi.SystemTextJson.UnitTests.ConformanceApi.fsd")!))
